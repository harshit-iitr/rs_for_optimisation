...
study(
    "S10_split_mnist",
    root="split_mnist/stiffness_curve",
    question="How does Radial Suppression perform on Class-Incremental Split MNIST?",
    claim="Supporting (generality and severe catastrophic forgetting).",
    seeds=SEEDS_CURVE,
    phases=[{"name": "sweep", "arms": [
        {"dir": _lam_dir(lam),
         "args": _canon(method="rs" if lam else "bp", lambda_rs=lam,
                        ...
                        width=256, depth=2, lr=0.01)}
        for lam in [0.0, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1.0, 3.0, 10.0]
    ]}],
)
...
study(
    "S11_split_mnist_til",
    root="split_mnist_til/stiffness_curve",
    question="How does Radial Suppression perform on Task-Incremental Split MNIST?",
    claim="Supporting (generality without output-layer interference).",
    seeds=SEEDS_CURVE,
    phases=[{"name": "sweep", "arms": [
        {"dir": _lam_dir(lam),
         "args": _canon(method="rs" if lam else "bp", lambda_rs=lam,
